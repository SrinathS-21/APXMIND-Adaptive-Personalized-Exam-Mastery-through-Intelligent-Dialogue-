...
Routes user queries to the appropriate agent via LLM structured output.
Migrated: LLM is now fetched lazily via get_llm(), not at module level.
"""
...
"""
    prompt = ChatPromptTemplate.from_messages(
        [("system", system_prompt), ("human", "Route the following user query: {question}")]
    )
...
def teacher_vectordb_router(state: State):
    ...
    system_prompt = (
        "You are an expert at routing a user's science question to the correct subject. "
        "Choose between 'biology', 'chemistry', or 'physics'."
    )
    prompt = ChatPromptTemplate.from_messages(
